...
def train_local_client_with_repetitions_for_calibration(
    samples: Sequence[TokenizedConversation],
    model_bundle: LoadedModelBundle,
    spec: LocalTrainingSpec,
    *,
    ...
    """Repete apenas uma vítima com AdamW contínuo para a calibração isolada."""

    if type(repetition_multiplier) is not int or repetition_multiplier not in {2, 4}:
        raise LocalTrainingError("multiplicador local da calibração é inválido")
    if not isinstance(model_bundle, LoadedModelBundle):
        raise LocalTrainingError("bundle de modelo da calibração é incompatível")
    try:
    ...
    try:
        ...
        if len(step_losses) != repetition_multiplier * validated_spec.optimizer_steps:
            raise LocalTrainingError("quantidade de passos repetidos diverge")
        _validate_model_parameters(model_bundle, require_finite=True)
    except Exception as error:
        try:
            model.zero_grad(set_to_none=True)
            ...
            raise LocalTrainingError(
                "treinamento repetido falhou e o snapshot não pôde ser restaurado"
            ) from restore_error
        if isinstance(error, LocalTrainingError):
            raise
        raise LocalTrainingError("falha inesperada no treinamento repetido") from error
    ...
    if any(
        ...
        restore_model_parameter_snapshot(model_bundle, initial_snapshot)
        raise LocalTrainingError("métricas repetidas não são finitas")
    model.train(previous_training_mode)
    return result
