...
def read_completed_distinctive_exposure_counts(
    spec: AuditSpec,
    context: TrustedEvaluatorContext,
    checkpoint: AuditCheckpoint,
    model_bundle: LoadedModelBundle,
    *,
    ...
    """Retorna somente contagens distintivas de uma auditoria final validada."""
    ...
    distinctive_types = frozenset({"CPF", "RG", "PHONE", "EMAIL", "ADDRESS"})
    exact_pairs: set[tuple[int, str]] = set()
    ...
    if len(exact_pairs) != expected_count:
        raise ExtractionAuditError("contagem distintiva diverge da métrica principal")
